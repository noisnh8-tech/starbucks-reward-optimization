...
→ 전체 리워드 비용의 **16.5%**가 마케팅 기여 없이 소진 — 오퍼 없이도 구매할 고객에게 불필요한 비용이 추가되는 구조
...
Python 탐색 분석 후, 반복 모니터링이 필요한 핵심 지표를 SQL로 별도 정리했습니다.  
SQLite 기반으로 실행 가능하며 `sql/` 폴더에서 확인할 수 있습니다.
